...
function, but for the purpose of this exercise we will keep truth also in the
same data file.
...
+ Working of the plugin. We will run data file provided in ~./data/~ and expect to
  see approximately code src_bash[:exports both]{diff <(cat data/instances.csv |
  cut -d, -f3) <(cat ./data/instances.csv | cut -d, -f2) | grep "^>" | wc -l}
  {{{results(=48=)}}} test failures.
